...
class CustomDatasetFromCSV(Dataset):
    def __init__(self, csv_path, height, width, transform=None):
        ...
        Args:
            ...
            transform: pytorch transforms for transforms and tensor conversion
        """
        self.data = pd.read_csv(csv_path, header=None).iloc[0:251][:]
...
class CustomDatasetFromCSVTest(Dataset):
    def __init__(self, csv_path, height, width, transform=None):
        """
            Args:
            csv_path (string): path to csv file
            ...
            transform: pytorch transforms for transforms and tensor conversion
            """
        self.data = pd.read_csv(csv_path, header=None).iloc[251:-1][:]
        self.labels = np.asarray(self.data.iloc[:, 0])
        self.height = height
        self.width = width
        self.transform = transform
...
custom_mnist_from_csv_test = CustomDatasetFromCSVTest('data_set.txt', 300, 300, transformations)

mn_dataset_loader = torch.utils.data.DataLoader(dataset=custom_mnist_from_csv,
                                                batch_size=batch_size,
                                                shuffle=True)

mn_dataset_loader_test = torch.utils.data.DataLoader(dataset=custom_mnist_from_csv_test,
                                                batch_size=342,
                                                shuffle=False)
...
def test():
    ...
    test_loss = 0
    correct = 0
    for data, target in mn_dataset_loader_test_test:
