...
class CustomDatasetFromCSVtest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_as_np = np.asarray(self.data.iloc[index][1:]).reshape(50, 50).astype('uint8')
        # Convert image from numpy array to PIL image, mode 'L' is for grayscale
        img_as_img = Image.fromarray(img_as_np)
        img_as_img = img_as_img.resize((300, 300), Image.ANTIALIAS)
...
custom_mnist_from_csv_test = CustomDatasetFromCSVtest('atmgray.csv', 50, 50, transformations)
